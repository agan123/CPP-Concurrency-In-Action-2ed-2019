...
之前介绍过阻塞调用，会阻塞一段不确定的时间，将线程挂起到等待的事件发生。很多情况下，这样的方式很不错，但是在一些情况下，就需要限制一下线程等待的时间。可以发送一些类似“我还存活”的信息，无论是对交互式用户，或是其他进程，亦或当用户放弃等待，并按下“取消”键直接终止等待。
...
所以，当`std::condition_variable`的两个成员函数wait_for()和wait_until()成员函数分别有两个重载，这两个重载都与wait()成员函数的重载相关——一个重载只等待发出信号，或超时过期，或出现虚假唤醒；另一个重载在唤醒时检查提供的谓词，并仅在提供的谓词为真（且条件变量已发出信号）或超时过期时返回。
...
当前时间可以通过调用clock类的静态成员函数now()获取；例如，`std::chrono::system_clock::now()`是将返回系统时钟的当前时间。某个时钟的时间点类型是通过time_point的typedef来确定的，所以some_clock::now()的类型就是some_clock::time_point。
...
基于时延的等待可由`std::chrono::duration<>`来完成，例如：可以最多等待35ms供future变为ready：
